...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-1-project-skeleton-ci-cd.md`
  ...
  evidence: |
    ...
    All `node_modules` edits are captured via `pnpm patch` (`pnpm-workspace.yaml`'s `patchedDependencies` + `patches/*.patch`), so they survive a fresh `pnpm install` rather than being silently lost. Verified with `./gradlew :app:tasks` reaching `BUILD SUCCESSFUL` with `installDebug` available; actual on-device install/launch (`pnpm android`) left for the PR author to run locally.
